  ...
  [:select, :offset].each do |method|
    ...
    context "no records in the table" do
      ...
      should "should return empty array" do
        ...
        assert_equal nil, Artist.random(nil, :method => method)
        assert_equal [], Artist.random(1, :method => method)
        assert_equal nil, Artist.limit(50).random(nil, :method => method)
      end
    ...
    context "1 record in the table" do
      ...
      should "select only 1 record even when you request more" do
        ...
        assert_equal @high_on_fire, Artist.random(nil, :method => method)
        assert_equal [@high_on_fire], Artist.random(1, :method => method)
        assert_equal [@high_on_fire], Artist.random(30, :method => method)
      end
      
      should "not return a record that doesnt match where" do
        assert_equal nil, Artist.where(:name => "Wang Is Burning").random(nil, :method => method)
      end
      ...
      context "3 records in table" do
        ...
        should "respect limits and orders" do
          assert_equal @fiona_apple, Artist.order("views desc").limit(1).random(nil, :method => method)
          assert_equal [@fiona_apple], Artist.order("views desc").limit(1).random(1, :method => method)
        end
        
        should "respect selecting certain columns" do
          assert_equal 3, Artist.find(@fiona_apple.id).views
          
          artists = Artist.select(:name).random(3, :method => method)
        ...
        should "respect scopes" do
          assert_equal [@fiona_apple], Artist.at_least_three_views.random(3, :method => method)
        end

        should "select all 3 if we want them" do
          random_artists = Artist.random(10, :method => method)
        ...
        context "with some albums" do
          ...
          should "work with includes" do
            artists =  Artist.includes(:albums).random(10, :method => method)
          ...
          should "work with joins" do
            albums = Album.joins(:artist).where("artists.views > 1").random(3, :method => method)
